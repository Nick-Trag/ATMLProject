...
def preprocess():
    ...
    print(gender)
    # drop not useful columns or columns with a lot of missing values
    ...
    X = X.dropna()
    # remove rows with not available values for gender
...
def label_powerset(X, labels):
    ...
    classifier = LabelPowerset(classifier=SVC(gamma='auto', random_state=0), require_dense=[False, True])
    print("fitting...")
    classifier.fit(x_train, y_train)
    print("predicting...")
    y_pred = classifier.predict(x_test)
...
def evaluation(y_test, y_pred):
    ...
    print('Precision ', str(precision_score(y_test, y_pred, average='samples')))
    print('Recall ', str(recall_score(y_test, y_pred, average='samples')))
